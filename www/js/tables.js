...
function displayScheduleTable() {
	...
	for (i = 0; i < numDays; i++) {
		...
		if (g.tempHistory[dstr] != undefined) {
			temp = Math.round(g.tempHistory[dstr]);
		}

		tableHTML +=
				' <td background="img/weather/' + g.iconHistory[dstr] + '.png"' + 'width="60px"><div data-table-col="' + i
						+ '" class="weather-icon" align="right"><b>' + temp + '</b></td> \r\n';
	}
	...
	for (z = 0; z < g.numZones; z++) {
		...
		for (i = 0; i < numDays; i++) {
			...
			var runTime = -1;
			if (g.runtimeHistory[dstr] != undefined) {
				runTime = Math.round(g.runtimeHistory[dstr][z]);
			}
...
function runZone(zoneName, runMins) {
	...
	sendRunZoneCmd(g.selectedZone, parseInt(runMins))
	g.running = true;
	g.runningZoneNum = g.selectedZone;
}
...
function onRunCancelButtonRunningClick() {
	sendRunZoneStopCmd(g.runningZoneNum);
...
function displayZoneConfigTable() {
	...
	for (z = 0; z < g.numZones; z++) {
		...
		tableHTML += '      <tr ' + oddStr + '> \r\n';
		tableHTML += '        <td><input type="text" class="name-input" value="' + g.zoneConf[z].Name + '"></td> \r\n';
	...
	tableHTML +=
			'      <td style="text-align: right">Airport code</td><td colspan="7" style="text-align: left; padding-left: 35px"><input type="text" id="airport_code" style="width: 80px;" value="'
					+ g.airportCode + '"></td> \r\n';
	tableHTML += '      </tr> \r\n';
	tableHTML += '      <tr class = "global-config-row" > \r\n';
	tableHTML +=
			'      <td style="text-align: right">Run time</td><td colspan="7" style="text-align: left; padding-left: 35px"><input type="text" id="runtime_input" style="width: 80px;" value="'
					+ g.runTime + '"></td> \r\n';
	tableHTML += '      </tr> \r\n';
...
function copyUIToConfig() {
	for (z = 0; z < g.numZones; z++) {
		znk = z.toString();
		zn = (z + 1).toString();
		g.globalConf.ZoneConfigs[znk].Name = $('#zone_config_table tr:eq(' + zn + ') td input.name-input').val();
		g.globalConf.ZoneConfigs[znk].Enabled = $('#zone_config_table tr:eq(' + zn + ') td input.run-checkbox').is(':checked');
		g.globalConf.ZoneConfigs[znk].GetsRain = $('#zone_config_table tr:eq(' + zn + ') td input.rain-checkbox').is(':checked');
		g.globalConf.ZoneConfigs[znk].ZoneETRate = parseFloat($('#zone_config_table tr:eq(' + zn + ') td input.etrate-input').val());
		g.globalConf.ZoneConfigs[znk].RunTimeMultiplier = parseFloat($('#zone_config_table tr:eq(' + zn + ') td input.runmult-input').val());
		g.globalConf.ZoneConfigs[znk].DepthIn = parseFloat($('#zone_config_table tr:eq(' + zn + ') td input.depthin-input').val());
		g.globalConf.ZoneConfigs[znk].MinVWC = parseFloat($('#zone_config_table tr:eq(' + zn + ') td input.minvwc-input').val());
		g.globalConf.ZoneConfigs[znk].MaxVWC = parseFloat($('#zone_config_table tr:eq(' + zn + ') td input.maxvwc-input').val());
	}
